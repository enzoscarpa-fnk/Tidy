...
const emit  = defineEmits<{ addSuggestedTag: [tag: string]; requestTypeOverride: [] }>()
...
const ENTITY_LABELS: Record<EntityType, string> = {
  AMOUNT: 'Montant', DATE: 'Date', VENDOR: 'Fournisseur', IBAN: 'IBAN', SIRET: 'SIRET',
}
...
<template>
  <section class="glass-panel p-4 border-tidy-mauve/20" aria-label="Détecté automatiquement">

    <!-- En-tête IA -->
    <div class="mb-3 flex items-center gap-2">
      <svg class="h-4 w-4 flex-shrink-0 text-tidy-mauve" viewBox="0 0 16 16" fill="currentColor" aria-hidden="true">
      ...
      <span class="text-xs font-semibold uppercase tracking-wide text-tidy-mauve">Détecté automatiquement</span>
    </div>

    <!-- Alerte détection incertaine -->
    <div v-if="isUncertain"
         class="mb-3 flex items-start gap-2 rounded-2xl bg-tidy-status-warning/10 border border-tidy-status-warning/20 px-3 py-2 text-xs text-tidy-status-warning"
         role="alert">
    ...
    <div v-if="displayedType" class="mb-3">
      ...
      <div class="flex flex-wrap items-center gap-2">
        <span class="rounded-full bg-tidy-mauve/15 px-3 py-1 text-sm font-semibold text-tidy-mauve">
          {{ displayedType }}
        </span>
        <span v-if="originalDetectedLabel" class="text-xs text-tidy-text-tertiary line-through" :aria-label="`Détection originale : ${originalDetectedLabel}`">
    ...
    <div v-if="filteredEntities.length > 0" class="mb-3">
      ...
      <dl class="grid grid-cols-2 gap-x-3 gap-y-2">
        <div v-for="entity in filteredEntities" :key="`${entity.entityType}-${entity.value}`" class="min-w-0">
          <dt class="text-xs text-tidy-text-secondary">{{ ENTITY_LABELS[entity.entityType] ?? entity.entityType }}</dt>
          <dd class="truncate text-sm font-medium text-tidy-text-primary">{{ entity.value }}</dd>
        </div>
      </dl>
    </div>
    ...
    <div v-if="intelligence.suggestedTags.length > 0">
      ...
      <div class="flex flex-wrap gap-1.5">
        <div v-for="tag in intelligence.suggestedTags" :key="tag" class="flex items-center gap-1">
          <span class="inline-flex items-center gap-1 rounded-full bg-tidy-mauve/15 px-2.5 py-0.5 text-xs font-medium text-tidy-mauve">
            <svg class="h-3 w-3 flex-shrink-0 opacity-70" viewBox="0 0 16 16" fill="currentColor" aria-label="Suggéré par l'IA"><path d="M8 1a.5.5 0 0 1 .45.28l1.57 3.18 3.51.51a.5.5 0 0 1 .28.85L11.3 8.26l.6 3.5a.5.5 0 0 1-.73.53L8 10.56l-3.14 1.73a.5.5 0 0 1-.73-.53l.6-3.5L2.19 5.82a.5.5 0 0 1 .28-.85l3.51-.51L7.55 1.28A.5.5 0 0 1 8 1z" /></svg>
            {{ tag }}
          </span>
          <button type="button"
                  class="text-xs text-tidy-mauve underline underline-offset-2 active:opacity-60"
