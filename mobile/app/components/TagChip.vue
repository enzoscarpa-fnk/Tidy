...
const emit = defineEmits<{ remove: [label: string] }>()

const baseClasses = computed(() =>
  props.variant === 'suggested'
    ? 'bg-tidy-mauve/15 text-tidy-mauve border-tidy-mauve/20'
    : 'bg-white/8 text-tidy-text-primary border-tidy-border-glass'
)
</script>
...
<template>
  <span
    ...
    :class="baseClasses"
  >
    <svg v-if="variant === 'suggested'" class="h-3 w-3 flex-shrink-0 opacity-70" viewBox="0 0 16 16" fill="currentColor" aria-label="Suggéré par l'IA">
    ...
    <button
      v-if="removable"
      type="button"
      class="-mr-0.5 ml-0.5 flex h-3.5 w-3.5 flex-shrink-0 items-center justify-center rounded-full transition-colors hover:bg-white/15 disabled:cursor-not-allowed disabled:opacity-40"
      :disabled="disabled"
