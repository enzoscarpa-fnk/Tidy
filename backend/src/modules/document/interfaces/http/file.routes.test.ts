vi.mock('../../../../infra/database/prisma.plugin', async () => {
  ...
  const prisma = {
    ...
    // Correction ici : gérer les transactions interactives et les tableaux
...
describe('document.routes.ts - Phase 4 checkpoint', () => {
  ...
  describe('Phase 5 - Sync Push/Pull (LWW)', () => {
    it('POST /api/v1/documents/sync gère le LWW : create, update, skip', async () => {
      const oldServerDate = new Date('2026-01-01T10:00:00Z');
      const seededDoc = testState.seedDocument({
        id: testState.nextUuid(),
        updatedAt: oldServerDate,
        title: 'Ancien Titre',
      });
      ...
      const payload = {
        documents: [
          ...
          // CAS 2 : Client plus ancien -> skip
          {
            id: seededDoc.id, // Simule un conflit sur le même ID (dans la vraie vie, ce serait une autre requête ou un autre appareil, mais ici on teste l'idempotence/rejet local)
            workspaceId: testState.WORKSPACE_ID,
      ...
      expect(response.body.data.results[1].status).toBe('skipped');
      expect(response.body.data.results[2].status).toBe('created');

      const updatedDoc = testState.documents.get(seededDoc.id);
      expect(updatedDoc?.title).toBe('Nouveau Titre LWW');

      const createdDoc = testState.documents.get(newDocId);
      expect(createdDoc?.title).toBe('Nouveau Doc');
    });

    it('GET /api/v1/documents/sync?since=0 retourne tous les documents (Pull complet)', async () => {
      testState.seedDocument({ id: testState.nextUuid(), syncedAt: new Date('2026-02-01T10:00:00Z') });
      ...
      const firstDoc = response.body.data.documents[0];
      expect(firstDoc).toHaveProperty('isDeleted');
      expect(firstDoc).toHaveProperty('notes');
      expect(firstDoc).toHaveProperty('textExtractionMethod');
    });

    it('GET /api/v1/documents/sync avec since=ISO8601 ne retourne que les modifiés', async () => {
      const pastDate = new Date('2026-01-01T10:00:00Z');
      const futureDate = new Date('2026-03-01T10:00:00Z');

      testState.seedDocument({ id: testState.nextUuid(), syncedAt: pastDate });
      const newDoc = testState.seedDocument({ id: testState.nextUuid(), syncedAt: futureDate });
      ...
      expect(response.body.data.documents[0].id).toBe(newDoc.id);
    });
  });
});
